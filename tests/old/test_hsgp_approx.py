...
def build_mmd_func(sample1, sample2):
    ...
    K = gpx.old.cov.ExpQuad(sample1.shape[1], ls=test_ell)
    Kxx = K(s1)
    Kyy = K(s2)
    Kxy = K(s1, s2)
...
class TestHSGP(_BaseFixtures):
    ...
    def test_set_boundaries_1d(self, x_min, x_max):
        ...
        c = 2
        L = gpx.old.hsgp_approx.set_boundary(X1s, c=c)
    ...
    def test_set_boundaries_3d(self, X2):
        ...
        L = gpx.old.hsgp_approx.set_boundary(X2s, c=2)
        assert np.all(L == 10)
    ...
    def test_mean_invariance(self):
        ...
        with pm.Model() as model:
            _ = pm.Data("X", X)
            cov_func = gpx.old.cov.ExpQuad(1, ls=3)
            gp = gpx.old.HSGP(m=[20], L=[10], cov_func=cov_func)
    ...
    def test_parametrization(self):
        ...
        with pytest.raises(ValueError, match="Provide one of `c` or `L`"):
            ...
            gpx.old.HSGP(m=[500], c=2, L=[12], cov_func=cov_func)

        with pytest.raises(ValueError, match=err_msg):
            # m must be a list
            cov_func = gpx.old.cov.ExpQuad(1, ls=0.1)
            gpx.old.HSGP(m=500, c=2, cov_func=cov_func)

        with pytest.raises(ValueError, match=err_msg):
            # m must have same length as L
            cov_func = gpx.old.cov.ExpQuad(2, ls=[1, 2])
            gpx.old.HSGP(m=[500], L=[12, 12], cov_func=cov_func)
        ...
        with pytest.raises(
            ...
            cov_func = gpx.old.cov.ExpQuad(2, ls=[1, 2])
            gpx.old.HSGP(m=[50, 50], L=[12, 12], parametrization="wrong", cov_func=cov_func)

        # pass without error, cov_func has 2 active dimensions, c given as scalar
        cov_func = gpx.old.cov.ExpQuad(3, ls=[1, 2], active_dims=[0, 2])
        gpx.old.HSGP(m=[50, 50], c=2, cov_func=cov_func)

        # pass without error, all have two dimensions
        cov_func = gpx.old.cov.ExpQuad(2, ls=[1, 2])
        gpx.old.HSGP(m=[50, 50], L=[12, 12], cov_func=cov_func)

    @pytest.mark.parametrize("cov_func", [gpx.old.cov.ExpQuad(1, ls=1)])
    @pytest.mark.parametrize("drop_first", [True, False])
    def test_parametrization_drop_first(self, model, cov_func, X1, drop_first):
        n_basis = 100
        with model:
            gp = gpx.old.HSGP(m=[n_basis], c=4.0, cov_func=cov_func, drop_first=drop_first)
            gp.prior("f1", X1)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (gpx.old.cov.ExpQuad(1, ls=1), "centered"),
            (gpx.old.cov.ExpQuad(1, ls=1), "noncentered"),
        ],
    )
    def test_prior(self, model, cov_func, X1, parametrization, rng):
        """Compare HSGP prior to unapproximated GP prior, gpx.old.Latent.  Draw samples from the
        prior and compare them using MMD two sample test.  Tests both centered and non-centered
        parametrization.
        """
        with model:
            hsgp = gpx.old.HSGP(m=[200], c=2.0, parametrization=parametrization, cov_func=cov_func)
    ...
    def test_conditional(self, model, cov_func, X1, parametrization):
        """Compare HSGP conditional to unapproximated GP prior, gpx.old.Latent.  Draw samples from the
        prior and compare them using MMD two sample test.  Tests both centered and non-centered
        parametrization.  The conditional should match the prior when no data is observed.
        """
        with model:
            hsgp = gpx.old.HSGP(m=[100], c=2.0, parametrization=parametrization, cov_func=cov_func)
            f = hsgp.prior("f", X=X1)
...
class TestHSGPPeriodic(_BaseFixtures):
    def test_parametrization(self):
        ...
        with pytest.raises(ValueError, match=err_msg):
            ...
            gpx.old.HSGPPeriodic(m=[500], cov_func=cov_func)

        with pytest.raises(ValueError, match=err_msg):
            # `m`` must be a positive integer
            cov_func = gpx.old.cov.Periodic(1, period=1, ls=0.1)
            gpx.old.HSGPPeriodic(m=-1, cov_func=cov_func)
        ...
        with pytest.raises(
            ...
            cov_func = 5.0 * gpx.old.cov.Periodic(1, period=1, ls=0.1)
            gpx.old.HSGPPeriodic(m=500, cov_func=cov_func)
        ...
        with pytest.raises(
            ...
            cov_func = gpx.old.cov.Periodic(2, period=1, ls=[1, 2])
            gpx.old.HSGPPeriodic(m=500, scale=0.5, cov_func=cov_func)

    @pytest.mark.parametrize("cov_func", [gpx.old.cov.Periodic(1, period=1, ls=1)])
    @pytest.mark.parametrize("eta", [100.0])
    @pytest.mark.xfail(
        reason="For `gpx.old.cov.Periodic`, this test does not pass.\
    ...
    def test_prior(self, model, cov_func, eta, X1, rng):
        """Compare HSGPPeriodic prior to unapproximated GP prior, gpx.old.Latent. Draw samples from the
        prior and compare them using MMD two sample test.
        """
        with model:
            hsgp = gpx.old.HSGPPeriodic(m=200, scale=eta, cov_func=cov_func)
            f1 = hsgp.prior("f1", X=X1)

            gp = gpx.old.Latent(cov_func=eta**2 * cov_func)
            f2 = gp.prior("f2", X=X1)
    ...
    def test_conditional_periodic(self, model, cov_func, X1):
        ...
        """
        with model:
            hsgp = gpx.old.HSGPPeriodic(m=100, cov_func=cov_func)
            f = hsgp.prior("f", X=X1)
            fc = hsgp.conditional("fc", Xnew=X1)
