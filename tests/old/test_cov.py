...
class TestCovAdd:
    def test_symadd_cov(self):
        ...
        with pm.Model() as model:
            cov1 = gpx.old.cov.ExpQuad(1, 0.1)
            cov2 = gpx.old.cov.ExpQuad(1, 0.1)
            cov = cov1 + cov2
        K = cov(X).eval()
    ...
    def test_rightadd_scalar(self):
        ...
        with pm.Model() as model:
            a = 1
            cov = gpx.old.cov.ExpQuad(1, 0.1) + a
        K = cov(X).eval()
    ...
    def test_leftadd_scalar(self):
        ...
        with pm.Model() as model:
            a = 1
            cov = a + gpx.old.cov.ExpQuad(1, 0.1)
        K = cov(X).eval()
        npt.assert_allclose(K[0, 1], 1.53940, atol=1e-3)
        # check diagonal
        Kd = cov(X, diag=True).eval()
    ...
    def test_rightadd_matrix(self):
        ...
        M = 2 * np.ones((10, 10))
        with pm.Model() as model:
            cov = gpx.old.cov.ExpQuad(1, 0.1) + M
        K = cov(X).eval()
    ...
    def test_leftadd_matrixt(self):
        ...
        M = 2 * pt.ones((10, 10))
        with pm.Model() as model:
            cov = M + gpx.old.cov.ExpQuad(1, 0.1)
        K = cov(X).eval()
        npt.assert_allclose(K[0, 1], 2.53940, atol=1e-3)
        # check diagonal
        Kd = cov(X, diag=True).eval()
    ...
    def test_leftprod_matrix(self):
        ...
        with pm.Model() as model:
            cov = M + gpx.old.cov.ExpQuad(1, 0.1)
            cov_true = gpx.old.cov.ExpQuad(1, 0.1) + M
        K = cov(X).eval()
    ...
    def test_inv_rightadd(self):
        ...
        with pytest.raises(ValueError, match=r"cannot combine"):
            cov = M + gpx.old.cov.ExpQuad(1, 1.0)

    def test_rightadd_whitenoise(self):
        X = np.linspace(0, 1, 10)[:, None]
        with pm.Model() as model:
            cov1 = gpx.old.cov.ExpQuad(1, 0.1)
            cov2 = gpx.old.cov.WhiteNoise(sigma=1)
            cov = cov1 + cov2
        K = cov(X).eval()
...
class TestCovProd:
    ...
    def test_multiops(self):
        ...
        with pm.Model() as model:
            cov1 = (
                3
                + gpx.old.cov.ExpQuad(1, 0.1)
                + M * gpx.old.cov.ExpQuad(1, 0.1) * M * gpx.old.cov.ExpQuad(1, 0.1)
            )
            cov2 = (
                gpx.old.cov.ExpQuad(1, 0.1) * M * gpx.old.cov.ExpQuad(1, 0.1) * M
                + gpx.old.cov.ExpQuad(1, 0.1)
                + 3
            )
        K1 = cov1(X).eval()
...
class TestCovPSD:
    def test_covpsd_add(self):
        ...
        with pm.Model() as model:
            cov1 = 2 * gpx.old.cov.ExpQuad(1, 0.1)
            cov2 = 5 * gpx.old.cov.ExpQuad(1, 1.0)
            cov = cov1 + cov2
    ...
    def test_copsd_multiply(self):
        ...
        omega = np.pi * np.arange(1, 101) / (2 * L)
        with pm.Model() as model:
            cov1 = 2 * gpx.old.cov.ExpQuad(1, ls=1)
            cov2 = gpx.old.cov.ExpQuad(1, ls=1)
    ...
    def test_covpsd_nonstationary1(self):
        ...
        omega = np.pi * np.arange(1, 101) / (2 * L)
        with pm.Model() as model:
            cov = 2 * gpx.old.cov.Linear(1, c=5)
    ...
    def test_covpsd_nonstationary2(self):
        ...
        omega = np.pi * np.arange(1, 101) / (2 * L)
        with pm.Model() as model:
            cov = 2 * gpx.old.cov.ExpQuad(1, ls=1) + 10.0
    ...
    def test_covpsd_notimplemented(self):
        class NewStationaryCov(gpx.old.cov.Stationary):
            pass
...
class TestCovKron:
    def test_symprod_cov(self):
        ...
        with pm.Model() as model:
            cov1 = gpx.old.cov.ExpQuad(1, 0.1)
            cov2 = gpx.old.cov.ExpQuad(1, 0.1)
            cov = gpx.old.cov.Kron([cov1, cov2])
        K = cov(X).eval()
    ...
    def test_multiops(self):
        ...
        with pm.Model() as model:
            cov1 = (
                3
                + gpx.old.cov.ExpQuad(1, 0.1)
                + gpx.old.cov.ExpQuad(1, 0.1) * gpx.old.cov.ExpQuad(1, 0.1)
            )
            cov2 = gpx.old.cov.ExpQuad(2, 0.1) * gpx.old.cov.ExpQuad(2, 0.1)
            cov = gpx.old.cov.Kron([cov1, cov2])
...
class TestCovSliceDim:
    def test_slice1(self):
        ...
        with pm.Model() as model:
            cov = gpx.old.cov.ExpQuad(3, 0.1, active_dims=[0, 0, 1])
        K = cov(X).eval()
    ...
    def test_slice2(self):
        X = np.linspace(0, 1, 30).reshape(10, 3)
        with pm.Model() as model:
            cov = gpx.old.cov.ExpQuad(3, ls=[0.1, 0.1], active_dims=[1, 2])
        K = cov(X).eval()
    ...
    def test_slice3(self):
        X = np.linspace(0, 1, 30).reshape(10, 3)
        with pm.Model() as model:
            cov = gpx.old.cov.ExpQuad(3, ls=np.array([0.1, 0.1]), active_dims=[1, 2])
        K = cov(X).eval()
        npt.assert_allclose(K[0, 1], 0.34295549, atol=1e-3)
        # check diagonal
        Kd = cov(X, diag=True).eval()
    ...
    def test_diffslice(self):
        X = np.linspace(0, 1, 30).reshape(10, 3)
        with pm.Model() as model:
            cov = gpx.old.cov.ExpQuad(3, ls=0.1, active_dims=[1, 0, 0]) + gpx.old.cov.ExpQuad(
                3, ls=[0.1, 0.2, 0.3]
            )
        K = cov(X).eval()
    ...
    def test_raises(self):
        ...
        with pytest.raises(ValueError):
            gpx.old.cov.ExpQuad(1, lengthscales, [True, False])
            gpx.old.cov.ExpQuad(2, lengthscales, [True])
...
class TestExpQuad:
    ...
    def test_2dard(self):
        X = np.linspace(0, 1, 10).reshape(5, 2)
        with pm.Model() as model:
            cov = gpx.old.cov.ExpQuad(2, np.array([1, 2]))
        K = cov(X).eval()
    ...
    def test_inv_lengthscale(self):
        X = np.linspace(0, 1, 10)[:, None]
        with pm.Model() as model:
            cov = gpx.old.cov.ExpQuad(1, ls_inv=10)
        K = cov(X).eval()
        npt.assert_allclose(K[0, 1], 0.53940, atol=1e-3)
        K = cov(X, X).eval()
        npt.assert_allclose(K[0, 1], 0.53940, atol=1e-3)
    ...
    def test_psd(self):
        ...
        test_1d_psd = (
            gpx.old.cov.ExpQuad(1, ls=ell).power_spectral_density(omega[:, None]).flatten().eval()
        )
    ...
    def test_euclidean_dist(self):
        ...
        with pm.Model():
            cov = gpx.old.cov.ExpQuad(1, ls=1)
...
class TestWhiteNoise:
    def test_1d(self):
        X = np.linspace(0, 1, 10)[:, None]
        with pm.Model() as model:
            cov = gpx.old.cov.WhiteNoise(sigma=0.5)
        K = cov(X).eval()
...
class TestRatQuad:
    def test_1d(self):
        X = np.linspace(0, 1, 10)[:, None]
        with pm.Model() as model:
            cov = gpx.old.cov.RatQuad(1, ls=0.1, alpha=0.5)
        K = cov(X).eval()
...
class TestExponential:
    def test_1d(self):
        X = np.linspace(0, 1, 10)[:, None]
        with pm.Model() as model:
            cov = gpx.old.cov.Exponential(1, 0.1)
        K = cov(X).eval()
...
class TestMatern52:
    def test_1d(self):
        X = np.linspace(0, 1, 10)[:, None]
        with pm.Model() as model:
            cov = gpx.old.cov.Matern52(1, 0.1)
        K = cov(X).eval()
    ...
    def test_psd(self):
        ...
        test_1d_psd = (
            gpx.old.cov.Matern52(1, ls=ell).power_spectral_density(omega[:, None]).flatten().eval()
        )
        npt.assert_allclose(true_1d_psd, test_1d_psd, atol=1e-5)
...
class TestMatern32:
    def test_1d(self):
        X = np.linspace(0, 1, 10)[:, None]
        with pm.Model() as model:
            cov = gpx.old.cov.Matern32(1, 0.1)
        K = cov(X).eval()
    ...
    def test_psd(self):
        ...
        test_1d_psd = (
            gpx.old.cov.Matern32(1, ls=ell).power_spectral_density(omega[:, None]).flatten().eval()
        )
        npt.assert_allclose(true_1d_psd, test_1d_psd, atol=1e-5)
...
class TestMatern12:
    def test_1d(self):
        X = np.linspace(0, 1, 10)[:, None]
        with pm.Model() as model:
            cov = gpx.old.cov.Matern12(1, 0.1)
        K = cov(X).eval()
...
class TestPeriodic:
    def test_1d(self):
        X = np.linspace(0, 1, 10)[:, None]
        with pm.Model() as model:
            cov = gpx.old.cov.Periodic(1, 0.1, 0.1)
        K = cov(X).eval()
    ...
    def test_psd(self):
        ...
        test_1d_psd = (
            gpx.old.cov.Periodic(1, period=P, ls=ell)
...
class TestPolynomial:
    def test_1d(self):
        X = np.linspace(0, 1, 10)[:, None]
        with pm.Model() as model:
            cov = gpx.old.cov.Polynomial(1, 0.5, 2, 0)
        K = cov(X).eval()
...
class TestWarpedInput:
    def test_1d(self):
        ...
        with pm.Model() as model:
            ...
            cov = gpx.old.cov.WarpedInput(1, warp_func=warp_func, args=(1, 10, 1), cov_func=cov_m52)
        K = cov(X).eval()
    ...
    def test_raises(self):
        ...
        with pytest.raises(TypeError):
            gpx.old.cov.WarpedInput(1, cov_m52, "str is not callable")
        with pytest.raises(TypeError):
            gpx.old.cov.WarpedInput(1, "str is not Covariance object", lambda x: x)
...
class TestWrappedPeriodic:
    def test_1d(self):
        X = np.linspace(0, 1, 10)[:, None]
        with pm.Model():
            cov1 = gpx.old.cov.Periodic(1, ls=0.2, period=1)
            cov2 = gpx.old.cov.WrappedPeriodic(
                cov_func=gpx.old.cov.ExpQuad(1, ls=0.2),
                period=1,
            )
        K1 = cov1(X).eval()
        K2 = cov2(X).eval()
    ...
    def test_raises(self):
        lin_cov = gpx.old.cov.Linear(1, c=1)
        with pytest.raises(TypeError, match="Must inherit from the Stationary class"):
            gpx.old.cov.WrappedPeriodic(lin_cov, period=1)
...
class TestGibbs:
    def test_1d(self):
        ...
        with pm.Model() as model:
            cov = gpx.old.cov.Gibbs(1, tanh_func, args=(0.05, 0.6, 0.4, 1.0))
        K = cov(X).eval()
    ...
    def test_raises(self):
        with pytest.raises(TypeError):
            gpx.old.cov.Gibbs(1, "str is not callable")
        with pytest.raises(NotImplementedError):
            gpx.old.cov.Gibbs(2, lambda x: x)
        with pytest.raises(NotImplementedError):
            gpx.old.cov.Gibbs(3, lambda x: x, active_dims=[0, 1])
...
class TestScaledCov:
    ...
    def test_raises(self):
        cov_m52 = gpx.old.cov.Matern52(1, 0.2)
        with pytest.raises(TypeError):
            gpx.old.cov.ScaledCov(1, cov_m52, "str is not callable")
        with pytest.raises(TypeError):
            gpx.old.cov.ScaledCov(1, "str is not Covariance object", lambda x: x)
...
class TestCircular:
    def test_1d_tau1(self):
        ...
        etalon = 0.600881
        with pm.Model():
            cov = gpx.old.cov.Circular(1, 1, tau=5)
        K = cov(X).eval()
    ...
    def test_1d_tau2(self):
        ...
        etalon = 0.691239
        with pm.Model():
            cov = gpx.old.cov.Circular(1, 1, tau=4)
        K = cov(X).eval()
        npt.assert_allclose(K[0, 1], etalon, atol=1e-3)
        K = cov(X, X).eval()
        npt.assert_allclose(K[0, 1], etalon, atol=1e-3)
...
class TestHandleArgs:
    def test_handleargs(self):
        ...
        a = 2
        b = 3
        func_noargs2 = gpx.old.cov.handle_args(func_noargs)
        func_onearg2 = gpx.old.cov.handle_args(func_onearg)
        func_twoarg2 = gpx.old.cov.handle_args(func_twoarg)
...
class TestCoregion:
    ...
    def test_full(self):
        ...
        with pm.Model() as model:
            B = gpx.old.cov.Coregion(2, W=self.W, kappa=self.kappa, active_dims=[0])
    ...
    def test_fullB(self):
        B_mat = self.B[self.rand_rows, self.rand_rows.T]
        with pm.Model() as model:
            B = gpx.old.cov.Coregion(1, B=self.B)
    ...
    def test_raises(self):
        with pm.Model() as model:
            with pytest.raises(ValueError):
                B = gpx.old.cov.Coregion(2, W=self.W, kappa=self.kappa)

    def test_raises2(self):
        with pm.Model() as model:
            with pytest.raises(ValueError):
                B = gpx.old.cov.Coregion(1, W=self.W, kappa=self.kappa, B=self.B)

    def test_raises3(self):
        with pm.Model() as model:
            with pytest.raises(ValueError):
                B = gpx.old.cov.Coregion(1)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["WrappedPeriodic", (gpx.old.cov.ExpQuad(1, 1.0), 1.0)],
...
def test_full_shape(kernel, args):
    X = np.arange(10)[:, None]
    Xs = np.arange(5)[:, None]
    assert tuple(getattr(gpx.old.cov, kernel)(*args).full(X, Xs).shape.eval()) == (len(X), len(Xs))
